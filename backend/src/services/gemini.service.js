...
        bookTitle, // NEW
        characterDetails, // NEW: an object for all character info
        sidekick, // NEW: an object for a companion character
        setting, // NEW: an object for location details
        inclusion, // NEW: an object for friends/family
        interests,
        genre,
        subgenre, // NEW
        tone, // NEW
...
    // NEW: Get character name from nested object, with fallback
    const actualCharacterName = (characterDetails?.name && characterDetails.name.trim() !== '') 
                                ? characterDetails.name.trim() 
                                : 'the protagonist';

    if (actualCharacterName === 'the protagonist' && (!characterDetails?.name || characterDetails.name.trim() === '')) {
...
    // NEW: Get pronoun defaults for the main character from the new characterDetails object
    const actualCharacterGender = (characterDetails?.gender && characterDetails.gender.trim() !== '') ? characterDetails.gender.trim() : 'non-binary';
    const characterPronounSubject = actualCharacterGender === 'male' ? 'he' : actualCharacterGender === 'female' ? 'she' : 'they';
    const characterPronounObject = actualCharacterGender === 'male' ? 'him' : actualCharacterGender === 'female' ? 'her' : 'them';
    const characterPronounPossessive = actualCharacterGender === 'male' ? 'his' : actualCharacterGender === 'female' ? 'her' : 'their';

    // NEW: Construct new prompt elements from the optional fields
    const characterAppearance = (characterDetails?.appearance && characterDetails.appearance.trim() !== '') ? `with a description of their appearance: ${characterDetails.appearance}.` : '';
    const characterTrait = (characterDetails?.trait && characterDetails.trait.trim() !== '') ? `Their personality is ${characterDetails.trait}.` : '';

    const sidekickDetails = (sidekick?.name && sidekick.name.trim() !== '') 
        ? `The main character has a sidekick named ${sidekick.name}, who is a ${sidekick.type || 'companion'}. Their personality is described as ${sidekick.trait || 'loyal and helpful'}.` 
        : '';

    const settingDetails = (setting?.location && setting.location.trim() !== '')
        ? `The story is set primarily in ${setting.location}.`
        : (setting?.vacationSpot && setting.vacationSpot.trim() !== '')
            ? `The story is set in a dream vacation spot: ${setting.vacationSpot}.`
            : '';

    const friendsAndFamily = (inclusion?.friends && inclusion.friends.length > 0)
        ? `The story should also include the following real people as supporting characters: ${inclusion.friends.map(f => `${f.name} (${f.relationship})`).join(', ')}.`
        : '';

    const themeAndTone = `The genre is ${safeGenre}, with a sub-genre of ${subgenre || 'general fiction'}. The tone should be ${tone || 'whimsical and lighthearted'}.`;

    const moralOrJoke = (moral && moral.trim() !== '')
        ? `The moral of the story should be about ${moral}.`
        : (insideJoke && insideJoke.trim() !== '')
            ? `Subtly incorporate this inside joke or memory into the narrative: "${insideJoke}".`
            : '';
...
    // --- Remaining-word budgeting logic (remains the same) ---
...
This is a character-driven narrative about ${actualCharacterName}. The reader ("${actualRecipientName}") is the audience and must never be mentioned or treated as a character in the story.
The title of the book is: "${bookTitle || 'A Personalized Tale'}".
...
STORY DETAILS:
- Main Character: ${actualCharacterName} ${characterAppearance} ${characterTrait}
- Sidekick/Companion: ${sidekickDetails}
- Setting: ${settingDetails}
- Supporting Characters: ${friendsAndFamily}
...
    // Payload and timeout logic remain the same
    const payload = {
...
            }
            console.error("Gemini API response missing chapter text or invalid structure:", JSON.stringify(result, null, 2));
