...
// REMOVED: FLAT_SHIPPING_RATES_AUD and getFlatShippingRate are no longer used here for dynamic shipping
// They will be kept as a fallback if the dynamic shipping feature needs to be disabled or fails.
...
        await client.query(bookSql, [bookId, userId, title, JSON.stringify(promptDetails), luluProductId, currentDate, currentDate, totalChaptersForBook]); // Fixed: Used promptDetails directly

        const firstChapterText = await generateStoryFromApi({
            ...promptDetails, // Fixed: Used promptDetails directly
            chapterNumber: 1,
...
            'pending',                                // $7
            parseFloat(finalPriceDollars.toFixed(2)), // $8 (total_price_usd)
            'USD',                                    // $9 (currency)
