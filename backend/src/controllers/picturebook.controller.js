...
// --- MODIFICATION: Import the correct queue from the single, consolidated queue service file ---
...
export const getPictureBooks = async (req, res) => {
    ...
    try {
        ...
        client = await pool.connect();
        const userId = req.userId;
        // --- MODIFICATION: Rewritten SQL to be clean and avoid syntax errors ---
        const sql = `SELECT pb.id, pb.title, pb.last_modified, pb.is_public, pb.user_cover_image_url, pb.lulu_product_id FROM picture_books AS pb WHERE pb.user_id = $1 ORDER BY pb.last_modified DESC`;
