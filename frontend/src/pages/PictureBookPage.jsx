...
function PictureBookPage() {
    ...
    // Auto-save title and author
    useDebouncedEffect(() => {
        if (!book || (title === book.title && author === book.author)) return Promise.resolve();
        const saveDetails = async () => {
            try {
                await apiClient.patch(`/picture-books/${bookId}`, { title, author });
                setBook(prevBook => ({...prevBook, title, author}));
            } catch (err) {
    ...
    const handleFinalizeAndPurchase = async () => {
        ...
        try {
            ...
            setSaveError("A problem occurred during finalization.");
        } finally {
                ...
                <div className="max-w-7xl mx-auto py-8 px-4">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex justify-between items-center mb-8">
                        ...
                        <div className="text-center">
                            <h1 className="text-2xl font-serif font-bold">{book?.title || 'Picture Book Editor'}</h1>
                    ...
                    <div className="mt-8 text-center">
                        <motion.button
                            ...
                            className="bg-teal-600 text-white font-bold py-4 px-10 rounded-lg hover:bg-teal-500 disabled:bg-slate-700 disabled:text-slate-400 transition-all shadow-lg text-lg"
                        >
                            {isFinalizing ? <LoadingSpinner text="Finalizing..." /> : 'Finalize & Purchase'}
                        </motion.button>
                        {!meetsPageRequirement && (
                            <p className="text-sm text-red-400 mt-2">A picture book must have exactly {REQUIRED_CONTENT_PAGES} pages. ({timeline.length}/{REQUIRED_CONTENT_PAGES})</p>
                        )}
                    </div>
                </div>
            </div>
