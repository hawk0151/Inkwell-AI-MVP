...
const MyOrdersPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchOrders = async () => {
            ...
            setError(null);
            try {
                // This is the simplified API call. The token is added automatically.
    ...
    const fetchLuluStatus = async (luluJobId) => {
        ...
        try {
            // This is the simplified API call.
            ...
            {!loading && orders.length === 0 ? (
                ...
                <ul className="space-y-6">
                    {orders.map((order) => {
                        ...
                        return (
                            <li key={order.id} className="border border-slate-700 p-6 rounded-lg shadow-lg bg-slate-800">
                                <div className="flex justify-between items-start mb-3">
                                    <h2 className="text-xl font-semibold text-green-400">{order.book_title || 'Unnamed Book'}</h2>
                                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${order.status === 'completed' || order.status === 'processing' ? 'bg-green-600' : 'bg-gray-600'}`}>
                                        {order.status ? order.status.charAt(0).toUpperCase() + order.status.slice(1) : 'Unknown'}
                                    </span>
                                </div>
                                <div className="space-y-2 text-gray-300 text-sm">
                                    <p>Order ID: <span className="font-mono">{order.id}</span></p>
                                    <p>Total Price: <span className="font-bold">{new Intl.NumberFormat('en-US', { style: 'currency', currency: order.currency || 'USD' }).format((order.total_cost || 0) / 100)}</span></p>
                                    <p>Ordered on: {order.created_at ? new Date(order.created_at).toLocaleString() : 'N/A'}</p>
                                </div>
                                {order.lulu_job_id && (
                                    <div className="mt-4 pt-4 border-t border-slate-700">
                                        <button onClick={() => fetchLuluStatus(order.lulu_job_id)} disabled={luluStatusInfo?.loading} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors disabled:bg-blue-400 disabled:cursor-wait">
