    ...
    return (
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            ...
            <motion.div 
                ...
                animate="visible"
            >
                <motion.div variants={itemVariants} className="bg-slate-800/50 p-8 rounded-lg shadow-lg text-center h-96 flex flex-col justify-center items-center">
                    <h2 className="text-5xl font-serif text-white">{book?.title}</h2>
                    <p className="mt-4 text-xl text-gray-300">by Inkwell AI</p>
                </motion.div>

                {timeline.map((event, index) => (
                    <motion.div key={index} variants={itemVariants} className="bg-slate-800/50 p-8 rounded-lg shadow-lg">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                            <div className={index % 2 === 0 ? 'md:order-1' : 'md:order-2'}>
                                {event.uploaded_image_url || event.image_url ? (
                                    <img src={event.uploaded_image_url || event.image_url} alt={event.event_date || 'Timeline image'} className="rounded-md object-cover w-full h-80" />
                                ) : (
                            ...
                            <div className={index % 2 === 0 ? 'md:order-2' : 'md:order-1'}>
                                ...
                                <p className="text-slate-300 leading-relaxed">{event.story_text}</p>
                            </div>
                        </div>
                    </motion.div>
                ))}
