...
function NovelPage() {
  ...
  useEffect(() => {
    ...
    else if (paramBookId === 'new' && location.state?.selectedProductId && !selectedProductForNew) {
    ...
    // Direct /novel/new with no selection state: show error
    else if ((paramBookId === 'new' || !paramBookId) && !location.state?.selectedProductId && !selectedProductForNew && isLoadingPage) {
      setError('To create a new novel, please select a book format first.');
      setIsLoadingPage(false);
    }
    // If product is selected but loadingPage still true, clear loading
    else if (paramBookId === 'new' && selectedProductForNew && isLoadingPage) {
      setIsLoadingPage(false);
    }
  }, [
    paramBookId,
